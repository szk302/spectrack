...
describe("buildContext", () => {
  it("基本的なコンテキストを構築する", () => {
    const doc = makeDoc();
    const ctx = buildContext({ config, doc, commandName: "add", cwd: "/repo" });
  ...
  it("args.file が未設定の場合は command.args に含めない", () => {
    const doc = makeDoc();
    const ctx = buildContext({ config, doc, commandName: "add", args: {}, cwd: "/repo" });
  ...
  it("options.deps が未設定の場合は command.options に含めない", () => {
    const doc = makeDoc();
    const ctx = buildContext({ config, doc, commandName: "add", options: {}, cwd: "/repo" });
  ...
  it("macro.nanoid は21文字のランダム文字列", () => {
    const doc = makeDoc();
    const ctx = buildContext({ config, doc, commandName: "add", cwd: "/repo" });
    expect(typeof ctx.macro.nanoid).toBe("string");
    expect(ctx.macro.nanoid.length).toBeGreaterThan(0);
  });
  ...
  it("options.upgradeDeps が boolean の場合は command.options に含める", () => {
    const doc = makeDoc();
    const ctx = buildContext({
      config,
      doc,
      commandName: "update",
      options: { upgradeDeps: true },
      cwd: "/repo",
    });
    expect(ctx.command.options.upgradeDeps).toBe(true);
  });
});
