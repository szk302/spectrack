...
履歴を遡る一部のコマンド（`log`, `diff`, `deps-diff`, `dependents --all` 等）を除き、すべてのドキュメントのパース・依存関係の解決・バージョン比較は**「Working Tree（手元の未コミット状態）」**を正として行います。
...
指定したドキュメントに「依存している」ドキュメント（逆引き）を検索する。仕様変更時の影響範囲の特定に使用する。

* **通常時**: 現在のWorking Treeのみを検索対象とし、高速に影響範囲を特定する。
* **`--all` オプション指定時**: `git log -S "[id]"` 等を用いて全コミット履歴を検索し、**過去に依存関係があったが現在は削除・変更されているファイル**も含めてすべての歴史的な依存元を特定する。
...
```console
$ spectrack dependents docs/prd/requirements.md

🔍 [st-prd-001] docs/prd/requirements.md に依存しているドキュメントを検索中...

  ✅ [st-uc-001] docs/use-case/UC001.md (2.1.3 @ Working tree)
...
```console
$ spectrack dependents docs/prd/requirements.md --all

🔍 [st-prd-001] docs/prd/requirements.md に依存した履歴を持つドキュメントを検索中...

  ✅ [st-uc-001] docs/use-case/UC001.md (現在も依存中)
  🕰️ [st-design-002] docs/design/old-arch.md (過去に依存。コミット a1b2c3d で依存解除/削除)
